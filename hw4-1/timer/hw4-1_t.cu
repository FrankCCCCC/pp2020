...
#define CLK 'c' // clock
// CUDA Timer
// 0: Computing, 1: H2D, 2: D2H
const int timer_len = 3;
...
double convert(struct timespec s, struct timespec e){
    if(!is_enable){return 0;}
...
double pause_rec(){
    ...
    struct timespec end, start = work;
    double accum = rec;
    clock_gettime(CLOCK_MONOTONIC, &(end));
    
    accum += convert(start, end);
...
void block_FW_cuda(int B) {
    int round = (n + B - 1) / B;
    cudaEvent_t ct_s, ct_e;
    cudaEventCreate(&ct_s);
    cudaEventCreate(&ct_e);
    cudaEventRecord(ct_s, 0);
    ...
    cudaEventRecord(ct_e, 0);
    float t = 0;
    // cudaEventSynchronize(stop[0]);
    // cudaEventElapsedTime(&t, start[0], stop[0]);
    cudaEventSynchronize(ct_e);
    cudaEventElapsedTime(&t, ct_s, ct_e);
    printf("T: %d\n", t);
}
